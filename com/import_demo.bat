...
REM  
REM NAME: import_demo.bat
REM
REM DESCRIPTION
REM   Demonstration of CSV data import into the Oracle database.
REM
REM   Search the received directory for CSV files named demo.
...
REM       If no errors:
REM         Load the data into the demo database table.
...
REM ---------------------------------------------------------------------------------------
REM 24/07/2022   Ian Bond      Created script
...
FOR /R %DATA_HOME%\RECEIVED %%F IN (DEMO*.CSV) DO ( 
  ...
  SQLPLUS %CONNECT_USER%/%CONNECT_PWD%@%DBCONNECT% @%APP_HOME%\SQL\IMPORT_DEMO.SQL "%%~NXF"
