...
REM
REM DESCRIPTION
REM   Import statistics data from CSV file into Oracle database.
REM
REM   Search the received directory for CSV files named stats.
...
REM ---------------------------------------------------------------------------------------
REM 10/03/2026   Ian Bond      Created script


REM Set the application environment variables
CALL ..\config\SET_ENV

FOR /R %DATA_HOME%\RECEIVED %%F IN (STATS*.CSV) DO ( 
  ...
  SQLPLUS %CONNECT_USER%/%CONNECT_PWD%@%DBCONNECT% @%APP_HOME%\SQL\IMPORT_STATS.SQL "%%~NXF"
